...
impl<'a> RyokanContext<'a> {
    ...
    pub fn run_tui(&mut self) -> Result<()> {
        ...
        loop {
            Self::draw_terminal(&mut terminal, &mut notes, &mut list_state, self.pin)?;
                ...
                match key.code {
                    ...
                    KeyCode::Char('n') => {
                        ...
                        notes = fs::read_dir(notes_dir_path)?
                            .filter_map(std::result::Result::ok)
                            .filter(|entry| {
                                entry.file_name().to_string_lossy().ends_with(".enc.txt")
                            })
                            .collect();
                    }
                    KeyCode::Down => {
                    ...
                    KeyCode::Enter => {
                            ...
                            && let Some(note) = notes.get(selected)
                        {
                            ...
                            let decrypted_content =
                                file::load_and_decrypt_note_content(&note_path, self.pin)?;
                            ...
                            let encrypted_content =
                                note::encrypt_note_content(&updated_content, self.pin)?;
